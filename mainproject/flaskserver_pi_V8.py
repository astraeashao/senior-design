...
STEP_Max = 200*13 #200 STEP = 360° When Count >= 200, CW = 0 and turn reverse
...
STEP_Backflag = False #True
testflag = False   #Test snoring wav
snoringcount = 0
writeflag = False
...
def motor_run():
    global DIR_PIN,STEP_PIN,CW,STEP_Action,STEP_Count,STEP_Max,STEP_Min,STEP_Blockflag,STEP_Blocktimer,STEP_Blocktime,STEP_Backflag
    while True:
        if (STEP_Blockflag == True) and (int(time.time()) - STEP_Blocktimer >= STEP_Blocktime):
            STEP_Blockflag = False #Cancel block
        if STEP_Backflag == True:
            STEP_Backflag = False
            CW = 1
            time.sleep(1.0)
...
def send_email(receiver,content,file):
    global email_user,email_pwd,email_host,email_port,email_sender
    ...
    mailserver.sendmail(email_sender, receiver, message.as_string())
    mailserver.quit()
    print("send success")
    ...
    while True:
        if tfstate == False:
            ...
            try:
                ...
                pass
def tflite_predict():
    global tfstate,audiolist,nowstate,laststate,tflite_model,snoringcount,STEP_Action,STEP_Blockflag,STEP_Blocktimer,STEP_Blocktime,STEP_Max,testflag
    ...
    while True:
                ...
                try:
                    ...
                    if classes[pred] == 'snoring':
                        ...
                        if STEP_Blockflag == False:
                            STEP_Blockflag = True
                            STEP_Blocktimer = (int(time.time()))
                            STEP_Action = STEP_Action + STEP_Max/8
                    else:
        ...
        if testflag == True:
            testflag = False
            try:
                avi_path = "snoring/snoringtest.wav"
                y, sr = librosa.load(avi_path)
                if y.shape[0] < sr:#padding zero
                    zero_padding = np.zeros(sr - y.shape[0], dtype=np.float32)
                    y = np.concatenate([y, zero_padding], axis=0)
...
def disable():
    ...
    if request.method == "GET":
        ...
        res = {"state": "ERROR"}
        return json.dumps(res)
@app.route('/gettfstate', methods=['get', 'post'])
def gettfstate():
    global tfstate
    if request.method == "GET":
        res = {"state": "OK","tfstate":str(tfstate)}
...
def startwrite():
    global writeflag,job_id2,filename
    if request.method == "GET":
        if writeflag == False:
            if os.path.exists(filename) == False:
                with open(filename,'a',newline='') as file:
                    ...
                    writer.writerow(['Time','Snoring'])
                    print("create")
            sched.add_job(writedata, 'interval', id=job_id2, minutes=1)
            sched.start()
            writeflag = True
            print("start")
            res = {"state": "OK"}
